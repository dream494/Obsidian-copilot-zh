  ...
  public async parseDoc(file: TFile, vault: Vault): Promise<MiyoParseResult> {
    ...
    try {
      ...
      if (typeof response.text !== "string" || response.text.trim().length === 0) {
        return { error: "Miyo parse-doc 返回了空文本" };
      }
  ...
  async parseFile(file: TFile, vault: Vault): Promise<string> {
    try {
      ...
      if (backend === "miyo-unavailable") {
        ...
        return `[错误：无法从 PDF ${file.basename} 提取内容。Miyo（本地文档处理器）不可用 — 请重新连接它，或在设置中将文档处理器切换为 Plus。]`;
      }
      ...
      if (backend === "miyo") {
        ...
        if (miyoResult && "error" in miyoResult) {
          ...
          return `[错误：无法从 PDF ${file.basename} 提取内容。${miyoResult.error}]`;
        }
      }
      ...
      return `[错误：无法从 PDF ${file.basename} 提取内容]`;
    }
  }
...
class CanvasParser implements FileParser {
  ...
  async parseFile(file: TFile, vault: Vault): Promise<string> {
    try {
      ...
      return `[错误：无法解析 Canvas 文件 ${file.basename}]`;
    }
  }
}
