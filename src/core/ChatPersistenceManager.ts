...
export class ChatPersistenceManager {
    ...
    try {
      ...
      if (existingFile && existingFileIsReal) {
        ...
        try {
          ...
          new Notice(`对话已保存为笔记：${preferredFileName}`);
          ...
          if (isFileAlreadyExistsError(error)) {
            ...
            try {
              ...
              new Notice(`对话已保存为笔记：${fallbackName}`);
              logWarn(
      ...
      new Notice("对话保存为笔记失败。请查看控制台了解详情。");
    }
  }
  ...
  async loadChat(file: TFile): Promise<ChatMessage[]> {
    try {
      ...
      new Notice("加载对话历史记录失败。请查看控制台了解详情。");
      return [];
    }
  }
