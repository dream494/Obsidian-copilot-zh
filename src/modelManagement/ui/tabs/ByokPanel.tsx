    ...
    <div className="tw-flex tw-flex-col tw-gap-4 tw-py-4">
      <div className="tw-flex tw-items-start tw-justify-between tw-gap-4">
        <div className="tw-flex tw-flex-col tw-gap-1">
          <div className="tw-text-xl tw-font-bold tw-text-normal">自带密钥（BYOK）</div>
          <div className="tw-max-w-xl tw-text-sm tw-text-muted">
            设置你自己的提供方和模型以在 Copilot 中使用。
          </div>
        </div>
      ...
      {selfHostOn && (
        <div className="tw-flex tw-items-start tw-gap-2 tw-rounded-lg tw-border tw-border-solid tw-px-3 tw-py-2.5 tw-text-xs tw-text-normal tw-bg-interactive-accent/10 tw-border-interactive-accent/30">
          ...
          <div className="tw-leading-relaxed">
            <span className="tw-font-semibold">自托管模式已开启。</span> 云提供方仍可使用，但会被标记并排列在你的本地和自托管端点之下 —— 它们的提示词会离开你的设备。是否使用由你决定。
          </div>
        </div>
      )}

      <SearchBar value={query} onChange={setQuery} placeholder="搜索提供方…" />

      <div className="tw-flex tw-flex-col">
        {loadState === "loading" ? (
          <div className="tw-text-sm tw-text-muted">正在加载目录…</div>
        ) : (
          <ByokGlobalTable
            groups={groups}
            emptyMessage={
              query.trim() && providers.length > 0 ? "没有符合搜索条件的提供方。" : undefined
            }
            onConfigure={(id) =>
