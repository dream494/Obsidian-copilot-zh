...
export const ByokGlobalTable: React.FC<ByokGlobalTableProps> = ({
  ...
  if (groups.length === 0) {
    return (
      ...
      >
        {emptyMessage ?? "还没有提供方 —— 点击 + 添加提供方开始。"}
      </div>
    );
  }
...
const ProviderCard: React.FC<ProviderCardProps> = ({
  ...
  const getStatusBadge = (): {
    ...
    if (!requiresKey) {
      return { label: "运行中", variant: "default", className: successClassName };
    }
    if (provider.apiKeyKeychainId) {
      return { label: "API 密钥已设置", variant: "default", className: successClassName };
    }
    return { label: "无密钥", variant: "secondary", className: "tw-rounded-full" };
  };
  ...
  const handleRemoveModel = async (configuredModelId: string, modelName: string): Promise<void> => {
    const modal = new ConfirmModal(
      ...
      `确定从 ${provider.displayName} 中移除 ${modelName}？`,
      "移除模型",
      "移除",
      "取消"
    );
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  aria-label={`${provider.displayName} 的更多操作`}
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        aria-label={`移除 ${model.info.displayName}`}
        tabIndex={0}
      >
