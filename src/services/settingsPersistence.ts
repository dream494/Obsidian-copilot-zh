...
function credentialMigrationFromRecovery(
  ...
  return {
    ...
    summary: "凭据存储已迁移到 Obsidian 钥匙串。",
    details: recovery.encrypted
      ? [
          `之前的密钥已复制到 ${recovery.path}。`,
          "部分密钥已加密且无法读回。请从相应提供方获取新密钥，并在设置中输入。",
        ]
      : [
          `之前的密钥已复制到 ${recovery.path}。`,
          "请在设置中重新输入密钥，然后删除备份文件。",
        ],
  };
}
...
export async function loadSettingsWithKeychain(
  ...
  if (JSON.stringify(rawSettings) !== JSON.stringify(diskSettings)) {
    ...
    if (backup.status === "failed") {
      ...
      credentialMigration = {
        ...
        title: "API 密钥",
        status: "error",
        summary: "API 密钥无法备份，因此 data.json 保持原样。",
        details: ["请检查仓库是否可写，然后重启 Obsidian 重试。"],
      };
      showFallbackNotice(
        "Copilot 无法备份存储在 data.json 中的 API 密钥，因此文件保持原样。请检查仓库是否可写，然后重启 Obsidian。"
      );
    } else {
      ...
      if (backup.status === "backed-up") {
        ...
        showFallbackNotice(
          backup.encrypted
            ? `Copilot 已将凭据存储迁移到 Obsidian 钥匙串。之前的密钥已复制到 ${backup.path}，但部分密钥已加密且无法读回。请从相应提供方获取新密钥，并在设置中输入。`
            : `Copilot 已将凭据存储迁移到 Obsidian 钥匙串。之前的密钥已复制到 ${backup.path}。请在设置中重新输入，然后删除该文件。`,
          15000
        );
      }
      try {
        ...
        markCredentialError(
          "请检查仓库是否可写，然后重启 Obsidian 重试。",
          "API 密钥已备份，但无法从 data.json 中移除。"
        );
        showFallbackNotice(
          "Copilot 无法从 data.json 中移除 API 密钥。请检查仓库是否可写，然后重启 Obsidian。"
        );
      }
    }
  }
  ...
  if (!keychain.isAvailable()) {
    markCredentialError(
      "此 Obsidian 版本不支持 Obsidian 钥匙串，因此密钥无法加载或保存。"
    );
    showFallbackNotice(
      "Obsidian 钥匙串不可用。Copilot 无法在此 Obsidian 版本中加载或保存 API 密钥。"
    );
  ...
  if (hadFailures) {
    markCredentialError(
      "部分 API 密钥无法从 Obsidian 钥匙串加载。如果问题持续存在，请重启 Obsidian。"
    );
    showFallbackNotice(
      "部分 API 密钥无法从 Obsidian 钥匙串加载。如果问题持续存在，请重启 Obsidian。"
    );
