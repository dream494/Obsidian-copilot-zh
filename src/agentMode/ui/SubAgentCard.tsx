      ...
      {inputPrompt ? (
        <div className="tw-my-1">
          <div
            ...
            role="button"
          >
            <span className="tw-flex-1 tw-truncate">提示词</span>
            {promptOpen ? (
...
function describeCounts(c: ChildCounts, truncated?: boolean): string {
  if (truncated) return "嵌套";
  const bits: string[] = [];
  if (c.tools > 0) bits.push(`${c.tools} 个工具`);
  if (c.reasoning > 0) bits.push(`${c.reasoning} 次推理`);
  return bits.length > 0 ? bits.join(" · ") : "无活动";
}
