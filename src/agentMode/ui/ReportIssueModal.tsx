  ...
  return (
    <div className="tw-flex tw-flex-col tw-gap-4">
      <div className="tw-flex tw-flex-col tw-gap-1">
        <span className="tw-text-sm tw-font-medium">出了什么问题？</span>
        <Textarea
          autoFocus
          value={note}
          onChange={(e) => setNote(e.target.value)}
          placeholder="描述你在做什么以及发生了什么…"
      ...
      {showOpencodeOption && (
        <label className="tw-flex tw-items-start tw-gap-2 tw-text-sm">
          <Checkbox
            ...
            className="tw-mt-0.5"
          />
          <span>包含 OpenCode 日志（有助于诊断后端问题）</span>
        </label>
      )}

      <div className="tw-flex tw-flex-col tw-gap-1.5 tw-text-sm">
        <span className="tw-font-medium">当你点击“准备报告”时</span>
        <ul className="tw-m-0 tw-flex tw-flex-col tw-gap-1 tw-pl-5 tw-text-muted">
          <li>
            智能体模式对话窗格的截图（不是整个屏幕）和最近的活动日志会保存到电脑上的一个文件夹中。
          </li>
          <li>该文件夹会被打开，同时浏览器中会打开一个已预填好的 GitHub issue。</li>
          <li>将保存的文件拖入该 issue 以附加，然后提交。</li>
        </ul>
      </div>
      ...
      >
        ...
        <div className="tw-flex-1">
          <span className="tw-block tw-font-semibold">在分享这些文件之前</span>
          <span className="tw-mt-0.5 tw-block tw-text-normal">
            活动日志可能包含你的提示词、笔记内容以及工具输入/输出（纯文本）。公开发布前请检查已保存的文件并移除任何敏感内容。
          </span>
        </div>
      </div>
...
export class ReportIssueModal extends Modal {
  ...
  constructor(private readonly params: ReportIssueModalParams) {
    ...
    this.setTitle("报告智能体模式问题");
  }

  onOpen() {
    if (!isDesktopRuntime()) {
      new Notice("仅桌面版支持报告问题。");
      this.close();
      return;
    }
  ...
  private async prepareReport(note: string, includeOpencodeLog: boolean): Promise<void> {
    ...
    try {
      ...
      new Notice("报告已就绪。请将保存的文件附加到刚打开的 GitHub issue 中。");
    } catch (err) {
      logError("[ReportIssue] failed to prepare report:", err);
      new Notice("准备问题报告失败。请查看控制台了解详情。");
    }
  }
}
