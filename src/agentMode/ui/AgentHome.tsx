...
const AgentHomeInternal: React.FC<AgentHomeProps> = ({
  ...
  const handleSaveAsNote = useCallback(async () => {
    try {
      ...
      if (result) {
        ...
        new Notice("还没有可保存的内容。");
      }
    } catch (error) {
      logError("[AgentMode] manual save failed", error);
      new Notice("将对话保存为笔记失败。请查看控制台了解详情。");
    }
  }, [manager]);
  ...
  useEffect(() => {
    if (isOrphanedProject) new Notice("此项目已不存在。");
  }, [isOrphanedProject]);
  ...
  const heroText = showProjectHero ? `在 ${projectName} 中对话` : greeting;
  const hero = (
