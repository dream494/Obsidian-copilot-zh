  ...
  ({ turn, app, value, onSelect }) => {
    ...
    return (
      <div className="tw-flex tw-flex-col tw-gap-2">
        <div role="tablist" aria-label="智能体回答" className="tw-flex tw-flex-wrap tw-gap-1">
          {options.map((option) => (
            <FanoutTab
              key={option.value}
              option={option}
...
const FanoutTurnBody: React.FC<FanoutTurnBodyProps> = ({ turn, value, app }) => {
  if (value === FANOUT_SUMMARY_OPTION) {
    ...
    switch (summaryDisplayState(turn)) {
      case "writing":
        return <FanoutStatusLine icon={<ThinkingSpinner />} text="正在撰写摘要…" shimmer />;
      case "waiting":
        return <FanoutStatusLine icon={<ThinkingSpinner />} text="正在等待回答…" shimmer />;
      case "cancelled":
        return (
          <FanoutStatusLine
    ...
    return (
      <FanoutTerminalState app={app} partialText={answer.text}>
        <FanoutStatusLine
          ...
          text={isError ? answer.error?.trim() || "该智能体未能回答。" : "已取消"}
    ...
    return (
      <div className="tw-flex tw-flex-col tw-gap-1">
        ...
        {answer.status === "running" ? (
          <FanoutStatusLine icon={<ThinkingSpinner />} text="正在输出…" shimmer />
        ) : null}
      </div>
    );
  }
  ...
  return <FanoutStatusLine icon={<ThinkingSpinner />} text="正在思考…" shimmer />;
};
