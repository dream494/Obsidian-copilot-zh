...
function useSessionDisplay(session: AgentSession) {
  ...
  return {
    ...
    descriptor,
    displayLabel: label ?? descriptor?.displayName ?? "对话",
  };
}
  ...
  return (
    ...
    >
      ...
      <Tooltip>
        ...
        <TooltipContent>新建智能体会话</TooltipContent>
      </Tooltip>
    </div>
  );
};
  ...
  return (
    <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} modal={false}>
      ...
      <DropdownMenuContent align="start" className="tw-w-32">
        ...
        <DropdownMenuItem onSelect={onClose}>关闭</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
};
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="icon"
          className="tw-my-1 tw-shrink-0"
          aria-label={`显示另外 ${sessions.length} 个会话`}
        >
