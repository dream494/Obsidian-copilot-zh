...
function genericToolLabel(part: ToolCallPart): string {
  ...
  return humanizeToolName(bare) || "工具调用";
}
...
const READ_SUMMARY: ToolSummary = {
  ...
  collapsedLine: (p, ctx) =>
    `${verb(p, "正在读取", "已读取")} ${displayTargetFromPath(p, ctx?.vaultBase ?? null) ?? targetFromTitle(p)}`,
  outcome: (p) => {
    const t = approxTokens(p);
    return t > 0 ? `约 ${formatTokens(t)} tokens` : null;
  },
...
const EDIT_SUMMARY: ToolSummary = {
  ...
  collapsedLine: (p, ctx) => {
    ...
    return `${verb(p, "正在编辑", "已编辑")} ${
      diffPathCount > 1
        ? pluralize(diffPathCount, "个文件", "个文件")
        : (displayTargetFromPath(p, ctx?.vaultBase ?? null) ?? "文件")
    }`;
  },
  outcome: (p) => {
    ...
    const desc =
      typeof input?.description === "string" && input.description.length > 0
        ...
        : typeof input?.prompt === "string"
          ...
          : targetFromTitle(p);
    return agent ? `${agent} · "${desc}"` : `子智能体 · "${desc}"`;
  },
  outcome: (p) => {
    ...
    if (progress.toolUses !== undefined) {
      bits.push(pluralize(progress.toolUses, "个工具", "个工具"));
    }
...
const KIND_DELETE_SUMMARY: ToolSummary = {
  ...
  collapsedLine: (p, ctx) =>
    `${verb(p, "正在删除", "已删除")} ${displayTargetFromPath(p, ctx?.vaultBase ?? null) ?? targetFromTitle(p)}`,
  outcome: () => null,
};
const KIND_MOVE_SUMMARY: ToolSummary = {
  icon: pickToolIcon({ toolKind: "move" }),
  collapsedLine: (p, ctx) =>
    `${verb(p, "正在移动", "已移动")} ${displayTargetFromPath(p, ctx?.vaultBase ?? null) ?? targetFromTitle(p)}`,
  outcome: () => null,
};
...
const KIND_THINK_SUMMARY: ToolSummary = {
  ...
  collapsedLine: (p) => verb(p, "正在思考", "已思考"),
  outcome: () => null,
};
