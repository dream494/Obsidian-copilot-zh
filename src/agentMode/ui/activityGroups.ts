...
function phraseFor(family: ActivityFamily, n: number): string {
  switch (family) {
    case "read":
      return `读取 ${pluralize(n, "个文件", "个文件")}`;
    case "edit":
      return `编辑 ${pluralize(n, "个文件", "个文件")}`;
    case "command":
      return `运行 ${pluralize(n, "个命令", "个命令")}`;
  }
}
...
export function summarizeActivity(
  ...
  if (thoughts > 0) {
    if (thinkingMs >= 1000) parts.push(`思考了 ${formatDuration(thinkingMs)}`);
    else if (parts.length === 0) parts.push("思考");
  }

  const line = parts.join(", ");
  return {
    line: line.length > 0 ? line.charAt(0).toUpperCase() + line.slice(1) : "已执行",
    failed,
  };
}
