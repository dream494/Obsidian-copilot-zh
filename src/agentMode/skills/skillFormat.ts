...
function splitFrontmatter(content: string): { yaml: string; body: string } {
  ...
  if (!m) {
    throw new SkillFormatError(
      "SKILL.md 必须以 --- 分隔的 YAML frontmatter 块开头"
    );
  }
...
export function parseSkillFile(content: string, parentDirName: string): ParsedSkillFile {
  ...
  if (doc.errors.length > 0) {
    throw new SkillFormatError(`SKILL.md 的 frontmatter YAML 无效：${doc.errors[0].message}`);
  }
  if (!isMap(doc.contents)) {
    throw new SkillFormatError("SKILL.md 的 frontmatter 必须是 YAML 映射");
  }

  const name = readString(doc, "name");
  if (!name) {
    throw new SkillFormatError("SKILL.md 的 frontmatter 缺少必填字段 `name`");
  }
  validateName(name, parentDirName);

  const description = readString(doc, "description");
  if (description === undefined) {
    throw new SkillFormatError("SKILL.md 的 frontmatter 缺少必填字段 `description`");
  }
...
export function validateName(name: string, parentDirName: string): void {
  if (typeof name !== "string" || name.length === 0) {
    throw new SkillFormatError("技能 `name` 必须是非空字符串");
  }
  if (name.length > NAME_MAX) {
    throw new SkillFormatError(
      `技能 \`name\` 最多 ${NAME_MAX} 个字符（当前为 ${name.length} 个）`
    );
  }
  if (!NAME_RE.test(name)) {
    throw new SkillFormatError(
      `技能 \`name\` 必须是小写字母 a–z、数字 0–9 和连字符，且不能以连字符开头、结尾或连续使用（当前为“${name}”）`
    );
  }
  if (name !== parentDirName) {
    throw new SkillFormatError(
      `技能 \`name\`（“${name}”）必须与父目录名称（“${parentDirName}”）一致`
    );
  }
}
...
export function validateDescription(description: string): void {
  if (typeof description !== "string" || description.length === 0) {
    throw new SkillFormatError("技能 `description` 必须是非空字符串");
  }
  if (description.length > DESCRIPTION_MAX) {
    throw new SkillFormatError(
      `技能 \`description\` 最多 ${DESCRIPTION_MAX} 个字符（当前为 ${description.length} 个）`
    );
  }
}
