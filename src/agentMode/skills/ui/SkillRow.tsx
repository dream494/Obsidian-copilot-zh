...
export const SkillRow: React.FC<SkillRowProps> = ({
  ...
  const lockdownTooltip = mirroredLockdown
    ? `此技能在 ${lockdownCount} 个智能体文件夹中有副本。` +
      `编辑其中一个副本会与其它副本悄悄产生差异。` +
      `请先将其迁移到你的共享文件夹以启用编辑。`
    : null;
      ...
      switch (decision.kind) {
        ...
        case "canonical-toggle": {
          ...
          if (!result.ok && result.code !== "eperm") {
            new Notice(
              `无法${decision.enabled ? "启用" : "禁用"} ${agent}：${result.message}`
            );
          }
          return;
        }
        case "mirrored-remove-one": {
          ...
          if (!result.ok) {
            new Notice(`无法从 ${decision.agent} 中移除 ${skill.name}：${result.message}`);
            return;
          }
          const survivors = (
          ...
          new Notice(
            `已将 ${skill.name} 从 ${decision.agent} 项目文件夹移除；在 ${survivorLabel} 中仍然生效。`
          );
          return;
        }
...
function buildLocationLabel(
  ...
  if (dirs.length === 1) {
    ...
    return rel !== undefined ? `位于 ${rel}` : null;
  }
...
function computeChips(skill: Skill): ChipSpec[] {
  ...
  if (skill.disableModelInvocation === true) {
    chips.push({ variant: "warn", label: "已关闭模型调用" });
  }
  if (skill.userInvocable === false) {
    chips.push({ variant: "default", label: "从 / 命令中隐藏" });
  }
...
function tooltipFor(agentName: string, enabled: boolean): string {
  return enabled ? `已为 ${agentName} 启用` : `已为 ${agentName} 禁用 · 点击启用`;
}
...
function runMigration(args: RunMigrationArgs): Promise<void> {
  ...
  try {
    ...
    new Notice(
      `无法迁移 ${args.skill.name}：${err instanceof Error ? err.message : String(err)}`
    );
  ...
  const commit = async (suppressFuture: boolean): Promise<void> => {
    ...
    if (!result.ok && result.reason !== "eperm") {
      new Notice(`无法迁移 ${args.skill.name}：${result.reason}`);
      return;
    }
    if (args.action === "expandToNewAgent") {
      new Notice(
        `已将 ${args.skill.name} 移至 ${canonicalFolderRel}/ 并启用 ${args.targetAgentDisplayName}。`
      );
    } else {
      new Notice(
        `已将 ${args.skill.name} 移至 ${canonicalFolderRel}/ 并禁用 ${args.targetAgentDisplayName}。`
      );
    }
  };
...
function runProactiveConsolidate(args: RunProactiveConsolidateArgs): Promise<void> {
  ...
  try {
    ...
    new Notice(
      `无法合并 ${args.skill.name}：${err instanceof Error ? err.message : String(err)}`
    );
    return Promise.resolve();
  }
  ...
  const commit = async (suppressFuture: boolean): Promise<void> => {
    ...
    if (!result.ok && result.reason !== "eperm") {
      new Notice(`无法合并 ${args.skill.name}：${result.reason}`);
      return;
    }
    new Notice(`已将 ${args.skill.name} 合并到 ${canonicalFolderRel}/。`);
  };
