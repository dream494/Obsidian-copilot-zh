...
export const EmptyPlaceholder: React.FC<EmptyPlaceholderProps> = ({ folder }) => {
  return (
    ...
    >
      ...
      <div className="tw-text-smaller tw-font-medium tw-text-muted">还没有技能</div>
      <div className="tw-max-w-[420px] tw-text-center tw-text-ui-smaller tw-text-faint">
        你创建或保存到共享文件夹中的技能会显示在这里。{" "}
        <code>.claude/skills/</code>、<code>.agents/skills/</code> 或{" "}
        <code>.opencode/skills/</code> 下的技能也会自动显示在这里。
      </div>
      <div className="tw-mt-3.5 tw-font-mono tw-text-smallest tw-text-faint">
        共享目录 · <code>&lt;vault&gt;/{folder}/</code>
      </div>
    </div>
  );
};
