    ...
    <div className="tw-flex tw-flex-col tw-gap-3">
      ...
      >
        ...
        <p className="tw-m-0 tw-text-ui-smaller tw-leading-relaxed tw-text-normal">
          {isProject
            ? `这将从你的项目中移除技能文件夹。仓库同步 / git 是唯一的回滚方式。`
            : "这将移除规范副本和所有智能体符号链接。仓库同步 / git 是唯一的回滚方式。"}
        </p>
      </div>

      <div className="tw-flex tw-flex-col tw-gap-2">
        <div className="tw-text-ui-smaller tw-text-muted">将移除：</div>
...
function collectDeletePaths(
  ...
  out.push({
    ...
    note: "规范 SKILL.md 及支持文件",
  });
  for (const agent of skill.enabledAgents) {
    const dir = agentDirsProjectRel[agent];
    if (dir === undefined) continue;
    out.push({ path: `${dir}/${skill.name}`, note: "符号链接" });
  }
  return out;
}
