  ...
  onOpen(): void {
    ...
    if (this.args.resolvedCanonicalName !== this.args.skill.name) {
      contentEl.createEl("p", {
        ...
        text:
          `你的共享文件夹中已有名称“${this.args.skill.name}”。` +
          `迁移后的技能将命名为“${this.args.resolvedCanonicalName}”。`,
      });
    }
    ...
    checkboxRow.createSpan({ text: "以后迁移不再询问" });
...
function buildTitle(args: MigrateSkillConfirmModalArgs): string {
  ...
  switch (args.variant) {
    case "project-single":
      return `将“${name}”移出以与 ${args.targetAgentDisplayName ?? "另一个智能体"} 共享？`;
    case "project-mirrored":
      return `合并“${name}”并与 ${args.targetAgentDisplayName ?? "另一个智能体"} 共享？`;
    case "disable-last-agent":
      return `在禁用 ${args.targetAgentDisplayName ?? "此智能体"} 之前将“${name}”移到你的共享文件夹？`;
    case "proactive-consolidate":
      return `将“${name}”合并到你的共享文件夹？`;
  }
}
...
function buildBody(args: MigrateSkillConfirmModalArgs): string {
  const target = args.targetAgentDisplayName ?? "另一个智能体";
  switch (args.variant) {
    case "project-single":
      return (
        `此技能目前只存在于你的 ${sourceAgentLabel(args)} 项目文件夹中。\n` +
        `要为其启用 ${target}，Copilot 需要将它移到共享位置，\n` +
        `以便两个智能体保持同步。`
      );
    case "project-mirrored":
      return `相同技能目前重复存在于 ${pluralFolders(args.sourceDuplicatePaths.length)}：`;
    case "disable-last-agent":
      return `此技能目前只存在于：`;
    case "proactive-consolidate":
      return `相同技能目前重复存在于：`;
...
function buildOutro(args: MigrateSkillConfirmModalArgs): string | null {
  const source = args.targetAgentDisplayName ?? "此智能体";
  switch (args.variant) {
    case "project-single":
      return (
        `你仍然可以像之前一样编辑技能 — 现在只有一个副本，\n` +
        `因此更改会立即对两个智能体可见。`
      );
    case "project-mirrored":
      return `此后，对技能的编辑将对全部 ${args.actionLines.filter((l) => l.verb === "创建").length} 个智能体可见。`;
    case "disable-last-agent":
      return (
        `如果禁用 ${source}，该文件在其项目文件夹中将无处存放。\n` +
        `Copilot 可以通过将技能移动到共享技能文件夹来保留它（不启用任何智能体）—\n` +
        `你可以随时重新启用智能体，或稍后在技能标签页中删除它。`
      );
    case "proactive-consolidate":
      return (
        `编辑、重命名或删除其中一个副本会与其它副本悄悄产生差异。\n` +
        `Copilot 可以将它们合并到单个共享位置，并在每个智能体文件夹中创建快捷方式，\n` +
        `以便所有智能体仍然看到该技能。\n\n` +
        `此后，你的编辑将自动在所有智能体之间保持同步。`
      );
  }
}
...
function buildConfirmLabel(args: MigrateSkillConfirmModalArgs): string {
  const target = args.targetAgentDisplayName ?? "智能体";
  switch (args.variant) {
    case "project-single":
    case "project-mirrored":
      return `移动并启用 ${target}`;
...
function sourceAgentLabel(args: MigrateSkillConfirmModalArgs): string {
  if (args.skill.location.kind !== "project") return "智能体";
...
function pluralFolders(n: number): string {
  if (n === 2) return "两个项目文件夹";
  if (n === 3) return "三个项目文件夹";
  return `${n} 个项目文件夹`;
}
