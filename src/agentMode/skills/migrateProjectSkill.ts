...
export async function migrateProjectSkill(
  ...
  try {
    ...
    return fail(`无法创建标准技能文件夹：${describe(err)}`);
  }
  ...
  try {
    ...
    return fail(`无法移动 ${sourceDirAbs} → ${newDirPath}：${describe(err)}`);
  }
  ...
  try {
    ...
    return { ok: false, reason: `无法序列化 SKILL.md：${describe(err)}` };
  }
  ...
  try {
    ...
    return { ok: false, reason: `无法写入 SKILL.md：${describe(err)}` };
  }
