...
export const ClaudeConfigView: React.FC<ClaudeConfigViewProps> = ({
  ...
  <ConfigDialogShell
    ...
    onClose={onClose}
  >
    <ConfigSection title="Claude Code 二进制文件">
      <p className="tw-my-0 tw-text-sm tw-text-muted">
        Copilot 会在本机运行 <code>claude</code> CLI。自动检测会检查常规安装位置。
      </p>
      <BinaryPathSetting
        binaryName="claude"
        ...
        notFoundHint="在已知安装位置未找到 claude。请运行下方的安装命令，然后再次点击自动检测。"
        onSave={onSavePath}
        onClear={onClearPath}
        persistOnAutoDetect
        detect={detect}
    ...
    <ConfigSection title="还没有安装？">
      ...
      <div className="tw-flex tw-flex-col tw-gap-4">
        ...
        <SetupStep index={2} title="登录">
          ...
          />
          <p className="tw-my-0 tw-text-sm tw-text-muted">
            Copilot 会继承 Claude Code CLI 持有的凭据 — 这里无需粘贴任何密钥。
          </p>
        </SetupStep>
      </div>
    </ConfigSection>
