...
export async function probeClaudeVersion(
  ...
  if (!version) {
    throw new Error(
      `无法读取已安装的 Claude Code 版本。Copilot 需要 Claude Code ${CLAUDE_MIN_VERSION} 或更高版本。`
    );
  }
  if (compareSemver(version, CLAUDE_MIN_VERSION) < 0) {
    return {
      ...
      message: `不支持 Claude Code ${version}。Copilot 需要 Claude Code ${CLAUDE_MIN_VERSION} 或更高版本。`,
    };
  }
