...
export function installBadge(state: InstallState): InstallBadgeSpec | null {
  if (state.kind === "ready") {
    return { label: "就绪", variant: "success", showCheck: true };
  }
  if (state.kind === "checking") {
    return { label: "正在检查…", variant: "outline" };
  }
  if (state.kind === "incompatible") {
    return { label: "版本不兼容", variant: "destructive", title: state.message };
  }
  if (state.kind === "error") {
    return { label: "错误", variant: "destructive", title: state.message };
  }
  // absent → no badge.
  return null;
}
...
const CONFIG_STATUS_BADGES: Record<InstallState["kind"], InstallBadgeSpec> = {
  ready: { label: "就绪", variant: "success", showCheck: true },
  absent: { label: "尚未配置", variant: "outline" },
  incompatible: { label: "需要更新", variant: "destructive", showAlert: true },
  checking: { label: "正在检查…", variant: "outline" },
