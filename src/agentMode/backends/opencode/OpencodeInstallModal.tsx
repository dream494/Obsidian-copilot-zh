...
const runningState = (e: ProgressEvent | null): OpencodeRunState => {
  if (!e) return { kind: "running", label: "正在启动…", percent: 0 };
  switch (e.phase) {
    case "resolve":
    ...
    case "download": {
      if (!e.total) {
        return {
          kind: "running",
          label: `正在下载 ${e.assetName} — ${formatBytes(e.received)}`,
          percent: 0,
        };
      }
      ...
      return {
        kind: "running",
        label: `正在下载 ${e.assetName} — ${formatBytes(e.received)} / ${formatBytes(e.total)} (${percent}%)`,
        percent,
      };
    }
    case "extract":
    ...
    case "done":
      return { kind: "running", label: "完成", percent: 100 };
  }
};
...
export const OpencodeConfigContainer: React.FC<{
  ...
  const install = React.useCallback(() => {
    manager
      ...
      .then(({ version }) => {
        ...
        new Notice(`opencode v${version} 已安装。`);
      })
      .catch((err: unknown) => {
    ...
    new ConfirmModal(
      ...
      async () => {
        try {
          ...
          forgetUpgradeOutcome();
          new Notice(`opencode 已卸载${bytes > 0 ? `（释放 ${formatBytes(bytes)}）` : ""}。`);
        } catch (e) {
          logError("[AgentMode] uninstall failed", e);
          new Notice(`卸载失败：${e instanceof Error ? e.message : String(e)}`);
        }
      },
      `移除所有已下载的 opencode 二进制文件${bytes > 0 ? `（${formatBytes(bytes)}）` : ""}，` +
        "包括仓库中的任何旧副本？你的自定义二进制文件路径和 BYOK 密钥将保留。",
      "卸载 opencode",
      "卸载"
    ).open();
  ...
  const upgrade = React.useCallback(() => {
    ...
    action
      .then(({ version }) => {
        ...
        new Notice(`opencode 已升级到 v${version}。`);
      })
      .catch((err: unknown) => {
  ...
  const saveCustomPath = React.useCallback(
    async (path: string): Promise<string | null> => {
      ...
      forgetUpgradeOutcome();
      new Notice("自定义 opencode 二进制文件路径已保存。");
      return null;
    },
    [manager, forgetUpgradeOutcome]
  );
  ...
  const clearCustomPath = React.useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      new Notice(`无法清除自定义路径：${e instanceof Error ? e.message : String(e)}`);
      return;
    }
    forgetUpgradeOutcome();
    new Notice("已清除自定义 opencode 路径。");
  }, [manager, forgetUpgradeOutcome]);
