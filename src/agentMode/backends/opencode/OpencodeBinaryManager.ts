...
export class OpencodeNotFoundError extends Error {
  constructor() {
    super("在常规安装位置或 PATH 中找不到 opencode。");
...
export function pickMatchingAsset(release: GithubRelease, candidates: string[]): GithubAsset {
  ...
  throw new Error(
    `未找到匹配的 opencode 发布资源。已尝试：${candidates.join(", ")}。` +
      `可用资源：${release.assets.map((a) => a.name).join(", ")}`
  );
}
...
export function toOpencodeInstallState(state: InstallState): BackendInstallState {
  ...
  return {
    ...
    message: `不支持 opencode v${state.version}。Copilot 需要 opencode v${OPENCODE_MIN_ACP_VERSION} 或更高版本。`,
  };
}
...
export class OpencodeBinaryManager {
  ...
  getDataDir(): string {
    ...
    if (!(adapter instanceof FileSystemAdapter)) {
      throw new Error("智能体模式需要桌面版 Obsidian（FileSystemAdapter）。");
    }
    ...
    if (!home || !nodePath().isAbsolute(home) || nodePath().parse(home).root === home) {
      throw new Error(
        "无法解析用于安装 opencode 运行时的主目录。" +
          "智能体模式会将其安装在 ~/.obsidian-copilot 下；请检查你的账户是否有有效的主目录。"
      );
    }
  ...
  private async installPipeline(
    ...
    opts.onProgress?.({ phase: "resolve", message: "正在解析平台资源…" });
    ...
    try {
      ...
      throw new Error(
        `无法在 ${dataDir} 创建 opencode 安装目录：` +
          `${e instanceof Error ? e.message : String(e)}。请检查该目录是否可写。`
      );
    }
    ...
    try {
      ...
      opts.onProgress?.({ phase: "extract", message: "正在解压归档…" });
  ...
  async upgradeCustomBinary(): Promise<{ version: string; path: string }> {
    return this.runExclusive({ kind: "busy" }, async () => {
      ...
      if (s.binarySource !== "custom" || !s.binaryPath) {
        throw new Error("未配置要升级的自定义 opencode 二进制文件。");
      }
      ...
      try {
        ...
        throw new Error(`\`${binaryPath} upgrade\` 失败：${err.message ?? String(err)}`);
      }
      const { stdout } = await verifyOpencodeBinary(binaryPath);
      const version = parseVersionFromStdout(stdout);
      if (!version) {
        throw new Error(`升级后 ${binaryPath} --version 未报告版本。`);
      }
      if (isOpencodeVersionOutdated(version)) {
        throw new Error(
          `opencode 升级未达到所需版本 ${OPENCODE_MIN_ACP_VERSION}+ ` +
            `（仍为 v${version}）。`
        );
      }
  ...
  private async writeCustomBinaryPath(p: string | null): Promise<void> {
    ...
    if (process.platform !== "win32") {
      try {
        ...
        throw new Error(`${p} 不可执行。请执行 chmod +x 后重试。`);
      }
    }
    ...
    if (!version) {
      throw new Error(
        `${p} --version 输出中未包含版本号。这是 opencode 二进制文件吗？`
      );
    }
  ...
  private async fetchReleaseMetadata(version: string): Promise<GithubRelease> {
    ...
    if (res.status === 403) {
      throw new Error(
        "GitHub API 已被限流（未认证请求为每小时 60 次）。请设置 GITHUB_TOKEN 或稍后重试。"
      );
    }
    if (res.status === 404) {
      throw new Error(`GitHub 上未找到 opencode v${version} 的发布版本。`);
    }
    if (res.status < 200 || res.status >= 300) {
      throw new Error(`获取 GitHub 发布版本失败，状态码为 ${res.status}`);
    }
...
async function locateFile(root: string, name: string): Promise<string> {
  ...
  throw new Error(`在 ${root} 下任何位置都未找到文件“${name}”`);
}
...
export async function verifyOpencodeBinary(p: string): Promise<{ stdout: string }> {
  try {
    ...
    if (err.signal === "SIGTERM") {
      throw new Error(
        `${p} 未在 ${VERIFY_BINARY_TIMEOUT_MS}ms 内响应 --version。这是 opencode 二进制文件吗？`
      );
    }
    throw new Error(
      `${p} --version 执行失败：${err.message ?? String(err)}。这是 opencode 二进制文件吗？`
    );
  }
}
