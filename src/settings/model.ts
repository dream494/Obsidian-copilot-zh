...
export function validateSkillsFolder(
  ...
  for (const segment of segments) {
    if (segment.length === 0) {
      return { ok: false, reason: "文件夹路径不能包含空段（//）。" };
    }
    if (segment === "..") {
      return { ok: false, reason: '文件夹路径不能包含 ".." 段。' };
    }
    if (segment === ".") {
      return { ok: false, reason: '文件夹路径不能包含 "." 段。' };
    }
    if (CONTROL_CHAR_RE.test(segment)) {
      return { ok: false, reason: "文件夹路径包含非法控制字符。" };
    }
    // Windows-illegal characters (rejected everywhere for portability).
    if (/[<>:"|?*]/.test(segment)) {
      return {
        ok: false,
        reason: "文件夹路径包含文件夹名称中不允许的字符（< > : \" | ? *）。",
      };
    }
  }
...
export function validateCopilotFolder(
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    return { ok: false, reason: "文件夹名称不能为空。" };
  }
  ...
  if (/^[/\\]/.test(trimmed) || /^[a-zA-Z]:/.test(trimmed)) {
    return { ok: false, reason: "文件夹路径必须相对于仓库根目录。" };
  }
  ...
  if (
    ...
    return {
      ok: false,
      reason: "文件夹路径不能使用 Obsidian 配置文件夹。",
    };
  }
  for (const segment of cleaned.split("/")) {
    ...
    if (/[. ]$/.test(segment)) {
      return { ok: false, reason: "文件夹名称不能以点或空格结尾。" };
    }
    if (WINDOWS_RESERVED_NAME_RE.test(segment)) {
      return { ok: false, reason: `"${segment}" 是 Windows 保留的名称。` };
    }
  }
  return { ok: true, folder: cleaned };
}
