...
const PATTERN_TYPE_CONFIG = {
  folder: { icon: Folder, label: "文件夹", colorClass: "tw-text-context-manager-yellow" },
  tag: { icon: Tag, label: "标签", colorClass: "tw-text-context-manager-orange" },
  note: { icon: FileText, label: "笔记", colorClass: "tw-text-context-manager-blue" },
  extension: { icon: Hash, label: "扩展名", colorClass: "tw-text-context-manager-green" },
} as const;
      ...
      <div className="tw-flex tw-flex-row tw-items-center tw-justify-between">
        ...
        {isOverflowing ? (
          ...
          >
            {isExpanded ? (
              <>
                显示更少 <ChevronUp className="tw-size-4 sm:tw-size-3" />
              </>
            ) : (
              <>
                显示 {allBadges.length} 项 <ChevronDown className="tw-size-4 sm:tw-size-3" />
              </>
            )}
          </Button>
        ) : (
