...
export const AgentSettings: React.FC = () => {
  ...
  const tabs: TabItemType[] = [
    ...
    { id: QUICK_CHAT_TAB_ID, icon: <MessageCircle className="tw-size-4" />, label: "快捷对话" },
  ];
  ...
  return (
    <section className="tw-space-y-4">
      <SettingSection label="智能体">
        <SettingItem
          type="select"
          title="默认后端"
          description="当你点击 + 开始新会话以及挂载时自动生成会话时使用。从模型选择器中选择模型也会更新此设置。"
  ...
  return (
    <SettingSection>
      <div className="tw-flex tw-min-w-0 tw-flex-col tw-py-4">
        <span className="tw-text-base tw-font-semibold">快捷对话模型</span>
        <span className="tw-text-xs tw-text-muted">
          显示在对话模型选择器中的模型。请在"模型（BYOK）"标签页添加提供方。
        </span>
      </div>
      <SettingItem
        type="select"
        title="默认模型"
        description="新对话开始时使用的模型。从你启用的快捷对话模型中选择。"
        value={resolvedDefaultModelId ?? "选择模型"}
        onChange={(value) => {
          if (value === "选择模型") return;
          updateSetting("defaultModelKey", value);
        }}
        options={
          hasDefault
            ? chatModelOptions
            : [{ label: "选择模型", value: "选择模型" }, ...chatModelOptions]
        }
        placeholder="模型"
      />
      ...
      {showCloudWarning && (
        <div className="tw-flex tw-items-start tw-gap-2 tw-rounded-lg tw-border tw-border-solid tw-px-3 tw-py-2.5 tw-text-xs tw-text-normal tw-bg-warning/10 tw-border-warning/40">
          ...
          <div className="tw-leading-relaxed">
            <span className="tw-font-semibold">云服务。</span>已开启自托管模式，但{" "}
            {descriptor.displayName} 在云端运行 —— 你的提示词会离开你的设备交给第三方。
            它仍然可用；只有在你接受这一点时才使用它。
          </div>
        </div>
      )}
        ...
        <div className="tw-flex tw-flex-col tw-gap-2 tw-py-4">
          ...
          {InlineInstall && (
            <div className="tw-text-xs tw-text-muted">
              可与 Copilot Plus 或你自己的 API 密钥配合使用 —— 请在 BYOK 标签页添加提供方。
            </div>
          )}
        </div>
