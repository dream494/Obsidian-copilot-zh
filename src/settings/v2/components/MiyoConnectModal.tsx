        ...
        <>
          <div className="tw-flex tw-flex-col tw-gap-2">
            ...
            <div className="tw-text-sm tw-text-muted">
              Copilot 无法连接本地 Miyo 实例。如果你还没安装 Miyo，请先下载；如果已安装，请打开应用，然后重试。
            </div>
          </div>
          <div className="tw-flex tw-gap-2">
            ...
            >
              下载 Miyo <ArrowUpRight className="tw-size-3.5" />
            </Button>
            <Button
              variant="secondary"
            ...
            >
              <MonitorDown className="tw-size-3.5" /> 打开 Miyo
            </Button>
          </div>
          <div className="tw-flex tw-items-center tw-justify-between">
            ...
            <Button
              ...
              onClick={onRetry}
            >
              <RefreshCw className="tw-size-3.5" /> 重试连接
            </Button>
          </div>
        </>
      )}
        ...
        <>
          <div className="tw-flex tw-flex-col tw-gap-2">
            ...
            <div className="tw-text-sm tw-text-muted">
              {canAutoAdd ? (
                <>
                  Miyo 正在运行。Copilot 将注册{" "}
                  <span className="tw-font-semibold tw-text-normal">此仓库</span>，以便 Miyo
                  在本地为其建立索引，用于搜索和对话 —— 无限制、不消耗积分。你注册的文件夹就是
                  Miyo 的工作边界；如果你打开 Miyo 的 Relay
                  连接器，ChatGPT 和 Claude 也可以访问此仓库。
                </>
              ) : (
                <>
                  Miyo 正在运行，但{" "}
                  <span className="tw-font-semibold tw-text-normal">此仓库</span>尚未添加。
                  请打开 Miyo，将此仓库作为文件夹添加，然后重试 —— Miyo 会在本地为它建立索引，
                  用于搜索和对话，无限制且不消耗积分。如果你打开 Miyo 的 Relay
                  连接器，ChatGPT 和 Claude 也可以访问它。
                </>
              )}
            </div>
            {addError && (
          ...
          {canAutoAdd ? (
            ...
            <div className="tw-flex tw-items-center tw-justify-end tw-gap-2">
              ...
              >
                {adding ? (
                  <>
                    <RefreshCw className="tw-size-3.5 tw-animate-spin" /> 正在注册…
                  </>
                ) : (
                  "注册并连接"
                )}
            ...
            <>
              ...
              <div className="tw-flex tw-items-center tw-justify-between">
                ...
                <Button
                  ...
                  onClick={onRetry}
                >
                  <RefreshCw className="tw-size-3.5" /> 重试
                </Button>
              </div>
            </>
          )}
