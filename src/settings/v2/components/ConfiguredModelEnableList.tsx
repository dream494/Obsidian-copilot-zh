...
export const ConfiguredModelEnableList: React.FC<ConfiguredModelEnableListProps> = ({
  ...
  const emptyState =
    descriptor.id === "opencode" ? (
      <span>
        尚未配置任何模型。请在{" "}
        <span className="tw-font-medium">模型（BYOK）</span>标签页添加提供方，或登录 opencode
        订阅，以在此处管理模型。
      </span>
    ) : (
      <span>
        尚未报告任何模型。请登录 / 安装 {descriptor.displayName} CLI 并重新加载，或与此智能体打开一次对话会话。
      </span>
    );
  ...
  return (
    <ModelEnableList
      ...
      searchPlaceholder={`搜索 ${descriptor.displayName} 模型…`}
