...
export const MiyoSettings: React.FC = () => {
  ...
  const handleResync = useCallback(async () => {
    ...
    try {
      ...
      switch (outcome) {
        case "verified":
          new Notice("Miyo 已与你的 Copilot 文件夹保持同步。");
          setServerScopeStale(false);
          break;
        case "resynced":
          new Notice("Miyo 已重新同步 —— 排除的文件夹已更新，已开始重新索引。");
          setServerScopeStale(false);
          break;
        case "resynced-scan-failed":
          new Notice("Miyo 已重新同步；索引将在 Miyo 下次扫描时跟上。");
          setServerScopeStale(false);
          break;
        case "resynced-grants-reset":
          ...
          new Notice(
            "Miyo 已重新同步。此文件夹的注册必须重建，因此其远程读写访问权限已被关闭 —— 如需使用，请在 Miyo 应用中重新启用。",
            10000
          );
          setServerScopeStale(false);
          break;
        case "conflict":
          new Notice(
            "Miyo 报告存在冲突的注册（可能使用了此仓库之前的名称）。请在 Miyo 应用中移除它，然后重试。"
          );
          break;
        case "unregistered":
          ...
          new Notice("此仓库尚未注册到 Miyo。请重新连接以完成注册。");
          setServerScopeStale(false);
          break;
        case "failed":
          new Notice("无法重新同步 Miyo。请确保 Miyo 正在运行，然后重试。");
          break;
      }
    } finally {
  ...
  const handleEvaluate = useCallback(async (): Promise<ConnectOutcome> => {
    ...
    if (outcome === "error" && mountedRef.current) {
      new Notice("无法确认此仓库与 Miyo 的连接，请重试。");
    }
    return outcome;
  }, [attemptConnection]);
      ...
      try {
        ...
        if (next) {
          ...
          if (result === "installed") {
            ...
            if (!unmounted()) new Notice("Miyo 搜索技能已安装");
          } else if (result === "collision") {
            if (!unmounted()) {
              new Notice(
                "你的技能文件夹中已存在名为“miyo-search”的技能。请重命名或移除它，然后重试。"
              );
            }
          } else if (!unmounted()) {
            new Notice("无法安装 Miyo 搜索技能，请重试。");
          }
        } else {
          ...
          if (result === "collision" && !unmounted()) {
            new Notice(
              "已禁用。用户创建的“miyo-search”技能保留在原处 —— 如果你不需要它，请手动移除。"
            );
          }
        }
      } catch {
        if (!superseded() && !unmounted()) {
          new Notice("无法更新 Miyo 搜索技能，请重试。");
        }
      } finally {
  ...
  return (
    <div className="tw-space-y-4">
      ...
      {(serverScopeStale === true ||
        ...
        <div className="tw-flex tw-items-center tw-gap-2 tw-rounded-lg tw-border tw-border-solid tw-px-3 tw-py-2.5 tw-text-xs tw-text-warning tw-bg-warning/10 tw-border-warning/30">
          ...
          <span className="tw-flex-1">
            Miyo 的排除文件夹与你的 Copilot 文件夹不匹配 —— 对话可能被索引并暴露。请重新同步以更新它们。
          </span>
          {canAutoAddVault() ? (
            <Button
              size="sm"
            ...
            >
              {resyncPending ? "正在重新同步…" : "重新同步 Miyo"}
            </Button>
          ) : (
            <span className="tw-shrink-0">请在 Miyo 应用中移除并重新添加此文件夹。</span>
          )}
        </div>
      )}

      {/* Connection */}
      <SettingSection label="连接">
        <CapabilityRow
          title="Miyo"
          description={
            <span>
              ...
              >
                下载 <ArrowUpRight className="tw-size-3.5" />
              </a>
            </span>
          }
          control={
        ...
        <div>
          ...
          {advancedOpen && (
            <div className="tw-pb-2">
              ...
              <SettingItem
                type="custom"
                title="远程 Miyo 服务器（高级）"
                description="留空以进行本地发现，或指向远程 Miyo 实例。上方的 Connector 使用此端点。"
              >
                <Input
                  value={urlDraft}
                  onChange={(event) => setUrlDraft(event.target.value)}
                  onBlur={commitUrl}
                  placeholder="留空以进行本地发现"
      ...
      <div className="tw-space-y-2">
        <div className="tw-text-xs tw-font-semibold tw-text-muted">由 Miyo 提供支持</div>
        <div className="tw-text-sm tw-text-muted">
          <span className="tw-font-semibold tw-text-normal">Plus</span> = Copilot 云端，消耗
          积分 · <span className="tw-font-semibold tw-text-accent">Miyo</span> = 本地，
          无限制，在你的设备上
        </div>
          ...
          <div className="tw-divide-y tw-divide-border">
            ...
            {(() => {
              ...
              return (
                ...
                >
                  <CapabilityRow
                    title="语义搜索"
                    description="理解语义而不仅仅是关键词 —— 在设备上找到相关笔记。"
                    control={
                      <SettingSwitch
            ...
            >
              ...
              <CapabilityRow
                ...
                description="仅当前仓库，还是 Miyo 已索引的全部内容。"
                control={
                  <SegmentedControl
                    aria-label="搜索范围"
                    options={[
                      { label: "当前仓库", value: "current" },
                      { label: "不限范围", value: "unrestricted" },
                    ]}
              ...
              <MiyoStatusRow
                title="搜索对话"
                description="在本地搜索你的 ChatGPT / Claude 对话。请在 Miyo 中设置对话来源和索引。"
                status={status.chatSync}
                statusText={chatSyncStatusText(status.chatSync)}
                actionLabel="在 Miyo 中管理"
            ...
            <div className="tw-px-4">
              <CapabilityRow
                title="文档处理器"
                description="通过 Miyo 在本地处理 PDF 和 EPUB；其他格式使用 Plus 云端。"
                control={
                  <SegmentedControl
                    aria-label="文档处理器后端"
                    options={[
