    ...
    },
    {
      ...
      onClick: (cmd) => {
        new ConfirmModal(
          app,
          () => onRemove(cmd),
          `确定要删除命令"${cmd.title}"吗？这将永久移除命令文件，无法撤销。`,
          "删除命令",
          "删除",
          "取消"
        ).open();
    ...
    <div className="tw-flex tw-flex-wrap tw-justify-around">
      <div className="tw-flex tw-items-center tw-justify-between tw-gap-2">
        <div className="tw-flex tw-items-center tw-gap-1">
          <span className="tw-text-sm tw-font-medium">在菜单中</span>
          <HelpTooltip
            content={
              <div className="tw-max-w-xs tw-text-xs">
                启用后，在编辑器中右键时，该命令将出现在上下文菜单中。
              </div>
            }
          />
        </div>
          ...
          <Button
            ...
            onClick={() => {
              new ConfirmModal(
                app,
                handleDelete,
                `确定要删除命令"${command.title}"吗？这将永久移除命令文件，无法撤销。`,
                "删除命令",
                "删除",
                "取消"
              ).open();
...
export const CommandSettings: React.FC = () => {
  ...
  const handleRemove = async (command: CustomCommand) => {
    try {
      ...
      new Notice(`命令"${command.title}"已成功删除！`);
    } catch (error) {
      logError("Failed to delete command:", error);
      new Notice("删除命令失败，请重试。");
      throw error;
    }
  };
      ...
      <section className={cn("tw-flex tw-flex-col tw-gap-4")}>
        <div className="tw-flex tw-flex-col tw-gap-2">
          ...
          <div className="tw-text-sm tw-text-muted">
            预设提示词，可从编辑器右键菜单或在对话中使用 <code>/</code>{" "}
            命令触发。
          </div>
        </div>
        ...
        <div className="tw-flex tw-items-start tw-gap-2 tw-rounded-md tw-border tw-border-solid tw-border-border tw-p-4 tw-text-muted tw-bg-interactive-accent/10">
          ...
          <div>
            命令会自动从自定义提示词文件夹{" "}
            <strong>{customPromptsFolder}</strong> 中的 .md 文件加载。修改这些文件也会更新命令设置。
          </div>
        </div>
        ...
        <SettingSection>
          <SettingItem
            type="switch"
            title="自定义提示词模板"
            description="在提示词中处理 {activenote}、{foldername} 或 {#tag} 等变量。需要原始提示词时请禁用。"
          ...
          <SettingItem
            ...
            options={[
              ...
              { label: "按字母顺序", value: PromptSortStrategy.ALPHABETICAL },
              { label: "手动", value: PromptSortStrategy.MANUAL },
            ]}
          />
        </SettingSection>
