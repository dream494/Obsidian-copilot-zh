...
const MISSING_LICENSE_MESSAGE =
  "使用 Symposium 发布前，请先在设置中添加 Copilot Plus 许可证密钥。";
const BUSY_MESSAGE = "本笔记已有 Symposium 操作正在进行。";
const INVALID_HANDOFF_PATH_MESSAGE =
  "Symposium 审查路径必须是当前仓库内的相对路径。";
...
function operationFailure(action: SymposiumAction, error: unknown): SymposiumFailureResult {
  ...
  return {
    kind: "failure",
    action,
    message: "Copilot 无法完成此 Symposium 操作。",
    accessNotice: false,
    retryable: true,
  };
}
...
function identityChangedFailure(action: SymposiumAction): SymposiumFailureResult {
  return {
    ...
    action,
    message:
      "本笔记的 Symposium 身份已变更。请先关闭并重新打开此对话框再重试。",
    accessNotice: false,
    retryable: false,
  };
}
...
function previewChangedFailure(action: SymposiumAction): SymposiumFailureResult {
  return {
    kind: "failure",
    action,
    message: "本地 HTML 预览已变更。请让智能体重新生成后再发布。",
    accessNotice: false,
    retryable: false,
  };
}
...
export class SymposiumPublisher {
    ...
    return this.withFileLock(file, lockAction, async () => {
      ...
      if (resolvedAction === "delete" && !docId) {
        return {
          kind: "failure",
          action: resolvedAction,
          message: "本笔记不再具有有效的 Symposium 链接。",
          accessNotice: false,
          retryable: false,
        };
      }
      ...
      try {
        ...
        if (currentDocId !== docId) {
          const result: SymposiumPersistenceResult = {
            ...
            action: "update",
            message:
              "原页面已更新，但本笔记的 Symposium 身份已变更或无法验证。其当前身份保持不变。",
            receipt,
          };
          if (!currentDocId) {
    ...
    try {
      ...
      if (!saved) {
        const result: SymposiumPersistenceResult = {
          ...
          action: "publish",
          message:
            "页面已公开，但本笔记的 Symposium 身份已变更或无法验证。其当前的 frontmatter 保持不变。",
          receipt,
        };
        let currentDocId: string | null | undefined;
        try {
    ...
    try {
      ...
      if (!removed) {
        ...
        return {
          ...
          action: "delete",
          message:
            "原页面已被撤回，但本笔记现在指向另一个 Symposium 文档。其较新的身份保持不变。",
        };
      }
      this.blockedPublishResults.delete(file);
  ...
  private deletePersistenceFailure(file: TFile, expectedDocId: string): SymposiumPersistenceResult {
    const result: SymposiumPersistenceResult = {
      ...
      action: "delete",
      message:
        "该公开页面已被删除。请重试从本笔记移除其链接，不会再次联系 Symposium。",
      retrySave: () =>
