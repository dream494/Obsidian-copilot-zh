...
export function QuickAskPanel({
  ...
  const handleReplace = useCallback(
    (messageId: string) => {
      ...
      if (!result.ok) {
        new Notice(result.message ?? "无法替换。");
        return;
      }
    ...
    >
      ...
      <div className="tw-px-3 tw-pb-1 tw-pt-2">
        <QuickAskInput
          ...
          placeholder={isStreaming ? "正在生成..." : "输入问题... "}
      ...
      <div className="tw-mt-auto tw-flex tw-items-center tw-justify-between tw-gap-2 tw-border-t tw-border-solid tw-border-border tw-px-3 tw-py-1.5">
        <div className="tw-flex tw-items-center tw-gap-1">
          ...
          <div className="tw-flex tw-items-center tw-gap-1.5">
            ...
            <HelpTooltip content="将活动笔记的内容作为上下文" side="top" />
          </div>
        </div>
        ...
        <div className="tw-flex tw-items-center tw-gap-1">
          ...
          {isStreaming ? (
            <Button variant="destructive" size="icon" onClick={stop} title="停止生成">
