      ...
      <div className="tw-relative tw-flex tw-min-h-0 tw-flex-1 tw-flex-col">
        ...
        {droppable && !isEmpty && (
          ...
          >
            ...
            <div className="tw-relative tw-text-xs tw-text-muted">作为包含项添加</div>
          </div>
        )}
      </div>
          ...
          {droppable ? (
            ...
            >
              ...
              <span className="tw-truncate">
                {isDragging ? "拖放到此处以添加上下文" : "将文件/文件夹拖到这里"}
              </span>
            </div>
          ) : (
            <span className="tw-truncate">添加一个网页/YouTube 链接</span>
          )}
          <AddUrlPopover
  ...
  return (
    <div className="tw-flex tw-flex-col tw-gap-2">
      <div className="tw-text-sm tw-text-muted">
        定义模式以将特定文件、文件夹、标签或属性包含到项目上下文中。你还可以添加网页或 YouTube 视频。
      </div>
      {editorBox}
    </div>
  );
