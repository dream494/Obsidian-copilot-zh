        ...
        <div className="tw-space-y-2">
          ...
          {skippedMarkdownCount > 0 && (
            <div className="tw-px-1 tw-text-ui-smaller tw-text-faint">
              {skippedMarkdownCount} 个 markdown 文件——无需
              转换
            </div>
          )}
        </div>
      ...
      {showHeader && (
        <div className="tw-space-y-1">
          <div className="tw-flex tw-items-center tw-gap-2">
            <h4 className="tw-text-sm tw-font-medium tw-text-normal">内容转换</h4>
          </div>
          <p className="tw-text-ui-smaller tw-text-muted">
            非 markdown 文件（PDF、图片、网页等）会被转换为文本以供 AI 使用。
          </p>
        </div>
      )}
              ...
              <CollapsibleTrigger asChild>
                ...
                >
                  <div className="tw-flex tw-items-center tw-gap-2">
                    ...
                    <span className="tw-text-ui-smaller tw-text-muted">{counts.total} 项</span>
                  </div>
                  {isExpanded ? (
