...
export class SourcesModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.empty();
    new Setting(contentEl).setName("来源").setHeading();
  ...
  private createSourceList(
    ...
    sources.forEach((source) => {
      ...
      link.title = `${displayText} - 拖拽以插入 wikilink`;
      link.draggable = true;
      ...
      if (typeof source.score === "number") {
        itemContainer.appendChild(
          this.contentEl.doc.createTextNode(` - 相关度得分：${source.score.toFixed(4)}`)
        );
      }
  ...
  private addExplanation(container: HTMLElement, explanation: unknown): HTMLElement {
    ...
    if (exp.lexicalMatches && (exp.lexicalMatches as unknown[]).length > 0) {
      ...
      details.push(
        `词法：在 ${Array.from(fields).join(", ")} 中匹配到 "${Array.from(queries).join('", "')}"`
      );
    }

    // Add semantic score
    if (exp.semanticScore !== undefined && (exp.semanticScore as number) > 0) {
      details.push(`语义：${((exp.semanticScore as number) * 100).toFixed(1)}% 相似度`);
    }
    ...
    if (exp.folderBoost) {
      ...
      details.push(
        `文件夹加权：${fb.boostFactor.toFixed(2)}x（${fb.folder || "根目录"} 中有 ${fb.documentCount} 篇文档）`
      );
    }
    ...
    if (exp.graphConnections) {
      ...
      if (gc.backlinks > 0) connectionParts.push(`${gc.backlinks} 个反向链接`);
      if (gc.coCitations > 0) connectionParts.push(`${gc.coCitations} 个共同引用`);
      if (gc.sharedTags > 0) connectionParts.push(`${gc.sharedTags} 个共享标签`);

      if (connectionParts.length > 0) {
        details.push(
          `图连接：${gc.score.toFixed(1)} 得分（${connectionParts.join("、")}）`
        );
      }
    }
    ...
    if (exp.graphBoost && !exp.graphConnections) {
      ...
      details.push(`图加权：${gb.boostFactor.toFixed(2)}x（${gb.connections} 个连接）`);
    }

    // Add base vs final score if boosted
    if (exp.baseScore !== exp.finalScore) {
      details.push(
        `得分：${(exp.baseScore as number).toFixed(4)} → ${(exp.finalScore as number).toFixed(4)}`
      );
    }
