...
function YoutubeTranscriptModalContent({ onClose }: { onClose: () => void }) {
  ...
  const validateInput = (inputUrl: string, updateState = true) => {
    ...
    if (validation.isValid) {
      ...
      if (updateState) {
        setError(validation.error || "无效的 URL");
        setIsValidUrl(false);
      }
      return { isValid: false };
    }
  ...
  return (
    <div className="tw-flex tw-flex-col tw-gap-4">
      ...
      <div className="tw-flex tw-flex-col tw-gap-2">
        <div className="tw-text-sm tw-text-muted">输入有效的 YouTube 视频 URL</div>
        <Input
...
export class YoutubeTranscriptModal extends Modal {
  ...
  constructor(app: App) {
    ...
    // @ts-ignore
    this.setTitle("下载 YouTube 转录（Plus）");
  }
