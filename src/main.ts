...
export default class CopilotPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.settingsUnsubscriber = subscribeToSettingsChange((prev, next) => {
      void (async () => {
        try {
          ...
          new Notice("Copilot 保存设置失败。请查看日志后重试。");
        }
    ...
    this.ribbonIconEl = this.addRibbonIcon(
      ...
      agentReady ? "打开 Copilot 智能体对话" : "打开 Copilot 对话",
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      void this.runStartupMigrations(isLegacyUpgrade).catch((error) => {
        ...
        new Notice("Copilot 无法完成启动迁移。请重新加载 Obsidian 后重试。");
      });
    });
  ...
  private async collectLegacyUpgradeRelocation(): Promise<StartupMigrationItem | null> {
    ...
    return {
      ...
      summary: "Copilot 现在将所有文件集中在一个文件夹下。现有文件不会被移动。",
      details: entries.map(
        ({ label, oldPath, newPath }) => `${label}：将 ${oldPath} 移动到 ${newPath}。`
      ),
    };
  }
  ...
  private async runStartupMigrations(isLegacyUpgrade: boolean): Promise<void> {
    ...
    const projectTask = task(
      this.projectRegister.initialize(),
      {
        ...
        title: "项目",
        status: "error",
        summary: "项目无法加载或迁移。请重新加载 Obsidian 后重试。",
      },
      "加载项目失败。请查看控制台了解详情。"
    );
    const commandsTask = task(
      this.customCommandRegister.initialize().then(() => migrateCommands(this.app)),
      {
        ...
        title: "自定义命令",
        status: "error",
        summary: "自定义命令无法加载或迁移。请重新加载 Obsidian 后重试。",
      }
    );
    const promptsTask = task(
      this.systemPromptRegister.initialize().then(() => migrateSystemPromptsFromSettings(this.app)),
      {
        ...
        title: "系统提示词",
        status: "error",
        summary: "系统提示词无法加载或迁移。请重新加载 Obsidian 后重试。",
      }
    );
    const relocationTask = task(this.collectLegacyUpgradeRelocation(), {
      ...
      title: "Copilot 文件夹",
      status: "error",
      summary: "文件夹目标无法准备。请重新加载 Obsidian 后重试。",
    });
    const license: StartupMigrationItem | null = needsLicenseReentry
      ? {
          ...
          title: "Copilot 许可证",
          status: "action-required",
          summary: "Copilot 在升级后无法恢复此前的付费状态。",
          details: ["请在 Copilot 设置中重新输入许可证密钥以恢复付费功能。"],
        }
      : null;
    if (isLegacyUpgrade) {
    ...
    await runStartupMigrationSummary({
      ...
      afterTasks: () => {
        ...
        if (!isLegacyUpgrade) {
          new Notice("Miyo 搜索需要重新同步 — 请打开 Miyo 设置选项卡。", 8000);
          return [license];
        }
        return [
          license,
          {
            ...
            title: "Miyo 搜索",
            status: "action-required",
            summary: "Copilot 文件夹更新后，Miyo 搜索需要重新同步。",
            details: ["打开 Miyo 设置选项卡以重新同步。"],
          },
        ];
      },
      present: (items) => {
  ...
  async newAgentChat(): Promise<void> {
    ...
    try {
      ...
      new Notice("创建智能体会话失败。请查看 Copilot 日志。");
    }
  }
  ...
  private requireAgentView(): AgentSessionManager | null {
    if (!isDesktopRuntime()) {
      new Notice("智能体对话在移动设备上不可用。");
      return null;
    }
    if (!this.agentSessionManager) {
      new Notice("智能体对话未初始化。");
      return null;
    }
  ...
  async openChatSourceFile(fileId: string): Promise<void> {
    if (isNativeChatId(fileId)) {
      new Notice(
        "此对话没有已保存的笔记。请开启“将对话自动保存为 Markdown”以将对话保存为库中的笔记。"
      );
      return;
    }
    ...
    if (file instanceof TFile) {
      ...
      new Notice(
        "无法从隐藏目录打开源文件。若要在编辑器中打开对话笔记，请在设置中将其保存到非隐藏文件夹。"
      );
    } else {
