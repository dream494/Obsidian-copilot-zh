...
export class SystemPromptRegister {
  ...
  private validatePromptReferences(availableTitles: Set<string>): void {
    ...
    if (
      ...
      new Notice(
        `默认系统提示词 "${settings.defaultSystemPromptTitle}" 在新文件夹中未找到。已清除默认选择。`
      );
    }
    ...
    if (selectedTitle && !availableTitles.has(selectedTitle)) {
      ...
      new Notice(
        `当前系统提示词 "${selectedTitle}" 在新文件夹中未找到。已清除对话选择。`
      );
    }
  }
  ...
  private handleFileDeletion = async (file: TAbstractFile) => {
    ...
    try {
      ...
      if (getSelectedPromptTitle() === file.basename) {
        setSelectedPromptTitle("");
        new Notice(`系统提示词 "${file.basename}" 已被删除。已清除当前对话选择。`);
      }
    } catch (error) {
      ...
      if (wasValidPromptFile) {
        ...
        if (oldFilename) {
          ...
          if (getSelectedPromptTitle() === oldFilename) {
            ...
            if (promptFile) {
              new Notice(
                `系统提示词 "${oldFilename}" 已重命名为 "${promptFile.basename}"。已更新当前对话选择。`
              );
            } else {
              new Notice(
                `系统提示词 "${oldFilename}" 已被移出提示词文件夹。已清除当前对话选择。`
              );
            }
          }
        }
