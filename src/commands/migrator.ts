...
export async function migrateCommands(app: App): Promise<StartupMigrationItem | null> {
  ...
  const details = [`存储在 ${getCustomCommandsFolder()} 中。`];
  if (unsupportedCommands.length > 0) {
    await saveUnsupportedCommands(app, unsupportedCommands);
    details.push(
      `${unsupportedCommands.length} 个不受支持的命令已保存到 ${getCustomCommandsFolder()}/unsupported。解决错误后，请将每个笔记移出该文件夹。`
    );
  }
  ...
  return {
    ...
    summary: `${commandsToMigrate.length} 个命令已迁移为笔记文件。`,
    details,
  };
}
